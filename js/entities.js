...
function updateOgre(e, game, dt) {
  ...
  if (e.state === 'patrol') {
    if (dToPlayer < seeR) e.state = 'chase';
    e.repathT -= dt;
    if (e.repathT <= 0) {
  ...
  // contact: cozy knockback, no damage
  if (dToPlayer < e.radius + p.radius + 4 && game.hitCooldown <= 0) {
    const kx = (p.x - e.x) / (dToPlayer || 1), ky = (p.y - e.y) / (dToPlayer || 1);
    p.knockX = kx * TILE * 9; p.knockY = ky * TILE * 9;
    game.hitCooldown = 1.2;
    game.shake(0.2, 0.02);
    game.freeze(0.06);
    game.burst(p.x, p.y - 12, 14, PALETTE.forest.ogreRed);
